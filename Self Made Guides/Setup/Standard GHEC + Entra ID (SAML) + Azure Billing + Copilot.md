...
|---|---|---|
| **Microsoft Entra Cloud Application Administrator or Application Administrator** | Creates the standard GitHub organization enterprise app, configures SAML, and starts org SCIM provisioning. | Microsoft Entra admin center → Entra ID → Enterprise apps → New application → search GitHub Enterprise Cloud - Organization → Create → Single sign-on → SAML → Basic SAML Configuration → Edit → set Identifier, Reply URL, and Sign on URL for the GitHub org → Save → SAML Certificates → download Certificate (Base64) → copy Login URL and Azure AD Identifier. Then Provisioning → Get started → Automatic → Admin Credentials → Tenant URL `https://api.github.com/scim/v2/organizations/ORG` and Secret Token → Test Connection → Save → Users and groups → Add user/group → Assign → Start provisioning. Handoff: SAML values, SCIM test success, and assigned pilot group. |
| **GitHub organization owner or dedicated SCIM setup user** | Enables org SAML, generates the org SCIM token, and keeps an active SAML session for provisioning setup. | GitHub → profile photo → Your organizations → [org] → Settings → Authentication security → SAML single sign-on → Enable SAML authentication → paste Sign on URL, Issuer, and Public Certificate → Test SAML configuration → Save → download recovery codes. For SCIM: visit `https://github.com/orgs/ORG/sso` as the setup user → Organization Settings → Authentication security → SCIM provisioning → Generate token or Enable SCIM → copy token once. Handoff: SAML test success, recovery codes, SCIM token, and Tenant URL. |
