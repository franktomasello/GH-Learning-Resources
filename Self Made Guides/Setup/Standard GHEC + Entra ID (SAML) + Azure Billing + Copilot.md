# 🚀 Guide to Setup GHEC (Standard / Non-EMU) + Microsoft Entra ID (SAML) + Attach Azure Subscription + Turn on Copilot
...
⚠️ **This user is required because GitHub's SCIM provisioning for Entra ID uses a personal access token (PAT) generated by a specific GitHub user.** If that user loses access or leaves the org, SCIM can stop working—so you want a stable, dedicated identity.
...
- 📌 The SCIM token is tied to this user—if the user is removed from the org, SCIM provisioning will break
...
💡 **In Standard non-EMU with Entra ID, SCIM provisioning uses a personal access token (PAT) or SCIM token generated from GitHub.** This token is entered into the Entra ID provisioning configuration to authorize the SCIM API connection.
...
💡 **Admin Consent:** If you don't see a "Permissions requested" prompt and instead see a message about needing admin approval, you may need to configure an admin consent workflow in Azure or work with your Entra ID global administrator.
...
🔐 **Recovery codes are your safety net if Entra ID becomes unavailable.**
